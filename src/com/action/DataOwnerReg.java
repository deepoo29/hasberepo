    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
      ...
      PreparedStatement pst = null;
        ...
        try{
            ...
            String sql="insert into user values('"+name+"','"+password+"','"+sex+"','"+age+"','"+emailid+"',now(),'"+phone+"','downer')";           
                System.out.println(">>"+sql); 
            pst = (PreparedStatement) conn.prepareStatement(sql);
             int a=pst.executeUpdate();
           if(a >0){
              response.sendRedirect("domAuthHome.jsp?msg=Registration Done");
            } else {
                response.sendRedirect("domAuthHome.jsp?msg=UserId Already Exit");
            }             
            }else{
             response.sendRedirect("domAuthHome.jsp?msg=Fill All The Fileds"); 
            } }catch(Exception e){
            response.sendRedirect("domAuthHome.jsp?msg=UserId Already Exit");
            e.printStackTrace();
    }
